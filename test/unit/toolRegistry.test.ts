...
test('request_consultant_callback: success flags the session and returns a success message', async () => {
    let flagged: string | null = null;
    const ctx = buildCtx({
        deps: {
            dynamics: fakeDynamics({ createCallbackRequest: async () => true }),
...
test('request_consultant_callback: returns an error when the CRM write fails', async () => {
    const ctx = buildCtx({ deps: { dynamics: fakeDynamics({ createCallbackRequest: async () => false }) } });
    const payload = JSON.parse(await runTool('request_consultant_callback', {}, ctx));
    assert.equal(payload.status, 'error');
    assert.ok(payload.message.includes("couldn't submit your request"));
});
